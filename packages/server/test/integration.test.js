...
test("serves HTTP/3 and WebTransport through the composed fallback server", {
	timeout: 15_000,
}, async () => {
...
test("serves concurrent HTTP/3 streams and accepts a WebTransport session", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
...
test("drains accepted work and rejects new streams on an existing connection", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
...
test("settles successful header, data, trailer, and header-FIN sends", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
...
test("releases completed response buffers during a long-lived stream", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const started = deferred();
...
test("streams public Response bodies and preserves repeated Set-Cookie fields", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	let chunks = 0;
...
test("ignores client-initiated unidirectional streams at the native boundary", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const started = deferred();
...
test("rejects WebTransport CONNECT when native has no session handler", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const started = deferred();
...
test("rejects WebTransport CONNECT when public API has no session handler", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
...
test("preserves repeated request fields and rejects configured header limits", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const started = deferred();
...
test("rejects malformed pseudo-headers and releases churned or abrupt peers", {
	timeout: 30_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const server = new HTTP3Server().handle({
...
test("backpressures slow HTTP request bodies until the reader accepts data", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const entered = deferred();
...
test("aborts cleanly while request-body receive completion is pending", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const started = deferred();
...
test("backpressures WebTransport stream data until its async handler settles", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const entered = deferred();
...
test("settles queued reliable sends when shutdown cancels a request", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const queued = deferred();
...
test("owns queued datagram buffers through terminal connection state", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const sent = deferred();
	const server = new HTTP3Server().handle({
		session(session) {
			setTimeout(() => {
				...
				const results = Array.from({ length: 512 }, () => session.sendDatagram(payload));
...
test("rejects reentrant native sends during WebTransport stream startup", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const started = deferred();
...
test("aborts pending WebTransport receive work at the drain deadline", {
	timeout: 15_000,
}, async () => {
	assert.ok(python, "AIOQUIC_PYTHON must name the pinned integration-client interpreter");
	const entered = deferred();
...
test("aborts hidden native work when TSFN delivery is rejected", {
	timeout: 15_000,
...
test("settles reliable sends when MSH3 submission fails", {
	timeout: 15_000,
