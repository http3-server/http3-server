...
export function pem(label, value) {
	let binary = "";
...
function normalizedCommonName(value) {
	if (
		typeof value !== "string" ||
		value.length === 0 ||
		textEncoder.encode(value).length > 64 ||
...
export async function createCertificate(options) {
	...
	if (!authority && dnsNames.length + ipAddresses.length === 0) {
		throw new TypeError("At least one DNS name or IP address is required");
	}

	const commonName = normalizedCommonName(options.commonName ?? dnsNames[0] ?? ipAddresses[0]);
	const issuerCommonName = normalizedCommonName(options.issuerCommonName ?? commonName);
	const validityDays = options.validityDays;
	const maximumValidityDays = options.maximumValidityDays;
	if (!Number.isInteger(validityDays) || validityDays < 1 || validityDays > maximumValidityDays) {
		throw new RangeError(
			`validityDays must be an integer from 1 through ${maximumValidityDays}`
		);
	}
	...
	const extensions = authority
		? sequence(
				extension("2.5.29.19", sequence(boolean(true)), true),
				extension("2.5.29.15", bitString(Uint8Array.of(0x06), 1), true)
			)
		: sequence(
				extension(
					"2.5.29.17",
	...
	const rawSignature = new Uint8Array(
		await globalThis.crypto.subtle.sign(
			...
			options.signingKey ?? keyPair.privateKey,
			tbsCertificate
		)
	);
